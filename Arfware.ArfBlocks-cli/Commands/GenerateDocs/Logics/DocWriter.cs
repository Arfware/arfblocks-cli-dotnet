...
internal class DocWriter
{
	...
	public static bool Write(Dictionary<Type, string> enums, List<DocumentedObject> documentedObjects, string outputPath)
	{
		if (RuntimeSettings.IsVerboseEnabled)
		...
		try
		{
			...
			content += Environment.NewLine + GetImports();

			content += Environment.NewLine + GetCustomTypes();

			content += Environment.NewLine + GetSettings();
			...
			foreach (var documentedObject in documentedObjects)
			{
				...
				foreach (var documentedEndpoint in documentedObject.DocumentedEndpoints)
				{
					...
					content += GetRequestPath(documentedEndpoint) + Environment.NewLine;
	...
	private static string GetRequestPath(DocumentedEndpoint documentedEndpoint)
	{
		var tabAsString = "\t\t\t";
		return $"{tabAsString}export const RequestPath = '{documentedEndpoint.RequestPath}';";
	}
